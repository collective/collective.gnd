...
class BeaconGnd(BrowserView):
    ...
    def gen_gnd_format(self):
        ...
        content_dict = [
            ...
            u'#MESSAGE: {0}'.format(self.settings.message),
            u'#FEED: {0}/beacon-gnd.txt'.format(self.portal_url),
            u'#CONTACT: {0}'.format(self.settings.contact),
            u'#INSTITUTION: {0}'.format(self.settings.institution),
            u'#DESCRIPTION: {0}'.format(self.settings.description),
